...
**포지션:** 점수 비례 배분, 단일 종목 최대 20%, 상위 10개
...
상세: `docs/backtest_results.md`, `docs/adaptive_strategy_results.md`
...
1. **유니버스 동적 구성** — 현재 하드코딩된 종목 → 시가총액 기반 동적 필터링 (구현 진행 중)
2. **풀 유니버스 백테스트** — S&P500 전체 + KOSPI/KOSDAQ 대상 (실행 중)
3. **API 라우터 통합 테스트** — FastAPI TestClient 기반 (미작성)
4. **Alembic 마이그레이션** — 스켈레톤만 존재, 마이그레이션 파일 미생성
5. **실서비스 데이터 파이프라인** — yfinance → KIS API (한국) / Alpha Vantage (미국)
6. **수익 모델 정의** — 구독형 vs 광고형
7. **법적 고지** — 투자 정보 제공 vs 투자 권유 구분
