...
> 최종 업데이트: 2026-04-09 (v3.1 개선사항 반영)
...
**대상 시장:** 미국 (S&P500 + NASDAQ-100, ~517종목) — 한국 시장 제외 (v3.1: INCLUDE_KR_MARKET = False)
...
**복합점수 (v3.1):**
```
점수 = ADX×0.35 + ret12m_skip1×0.30 + 섹터ETF초과수익률×0.20 + 거래량안정성×0.15
```
- `ret12m_skip1`: 12개월 수익률에서 최근 1개월 제외 (단기 과매수 회피, v3.0의 3개월 수익률에서 변경)
- ADX 가중치: 0.40 → 0.35 / 변동성안정성: 0.10 → 0.15

**포지션 (v3.1):**
- 점수 × sqrt(시가총액) 가중 배분 (시총 가중치 신규 도입)
- 단일 종목 최대 10%, 상위 10개
- VOL_TARGET = 0.15 (변동성 스케일링: 포트폴리오 목표 변동성 15%)
- BUY_HOLD_SPREAD = 2.5 (매수 임계점 > 보유 임계점 → 회전율 41% 감소)

**시장 레짐 필터 (v3.1 신규):**
```
REGIME_FILTER_MODE:
  info   — 레짐 상태를 로그/리포트에 표시만 함 (기본값)
  block  — 약세장 레짐 시 신규 매수 차단
  off    — 레짐 필터 비활성화
```

**배포 환경 분기 (v3.1 신규):**
```
DEPLOY_ENV=serverless  # GitHub Pages (정적 JSON)
DEPLOY_ENV=local       # 로컬 개발 (.venv)
DEPLOY_ENV=cloud       # Docker / ECS
```

**스크리닝 유니버스 (v3.1):** S&P 500 + NASDAQ-100 동적 수집 (~517개, KR 제외)
...
### v3.0 (2015~2026-03, KR 포함, 격주×A진입방식)

| 전략 | CAGR | MDD | 샤프 | 거래 수 |
|---|---|---|---|---|
| 공격적 (ATR1.5, TOP15) | +48.1% | -1.3% | 3.94 | — |
| 균형형 (ATR2.0, TOP10) | +56.8% | -1.5% | 3.96 | — |
| 보수적 (ATR2.5, TOP7)  | +63.5% | -3.6%* | 3.92 | 2,888 |
| 적응형 (ATR2.0, TOP10) | +49.0% | -3.2% | 3.76 | — |
| SPY | +12.6% | -31.0% | 0.82 | — |

*보수적 전략 기준 v3.0 수치 (비교용)

### v3.1 (2015~2026-03, KR 제외, mcap가중, BuyHoldSpread)

| 전략 | CAGR | MDD | 샤프 | 거래 수 |
|---|---|---|---|---|
| 보수적 (ATR2.5, TOP7) | **+46.5%** | **-2.7%** | **3.62** | **1,703** (↓41%) |
