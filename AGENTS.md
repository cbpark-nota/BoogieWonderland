...
11. 코드를 실행할 때 터미널에 출력되는 내용은 최소한이 되도록 코드를 작성한다. 진행 상황 로그, 디버그 출력, verbose 메시지 등은 기본적으로 비활성화하고, 꼭 필요한 결과만 출력한다.
...
## 브랜치 관리
- 별도의 지시가 없다면 항상 `develop` 브랜치에서 작업을 시작한다.
- 현재 브랜치가 `develop`이 아니면, 먼저 브랜치 상태를 확인하고 `develop`으로 전환 가능한지 확인한 뒤 작업한다.
- 별도의 지시가 없다면 `main` 브랜치에서는 직접 작업하거나 직접 커밋하지 않는다.
- 기능 수정이나 실험이 필요하면 작업 브랜치를 생성할 수 있지만, 기본 머지 대상은 항상 `develop`이다.
- `main` 브랜치 머지 또는 `main` 직접 작업은 사용자가 명시적으로 요청한 경우에만 수행한다.
- `develop`, `main` 이외의 브랜치는 merge 후 삭제한다.
- worktree 브랜치(Codex/* 등)도 작업 완료 및 merge 후 즉시 삭제한다.
