...
3. 스크립트 실행 전 반드시 `uname -s`로 환경을 확인하고, 환경에 맞는 방식으로 실행한다.
...
9. commit 전에는 항상 테스트 코드를 실행하고, 테스트를 통과하지 못 하는 경우 디버깅을 시도한다.
10. worktree 또는 별도 브랜치에서 작업 시, push 전에 반드시 main의 최신 변경사항을 rebase/merge한 후 테스트를 실행한다.
