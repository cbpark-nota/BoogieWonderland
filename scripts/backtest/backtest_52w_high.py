...
if __name__ == "__main__":
    ...
    logger.debug("=" * 70)
    logger.debug("  52주 신고가 돌파 전략 백테스트")
    logger.debug(f"  기간       : {START} ~ {END}")
    logger.debug(f"  수수료     : 편도 {COMMISSION*100:.1f}%")
    logger.debug(f"  유니버스   : 풀 유니버스 (S&P500 + NASDAQ-100 + KOSPI200 + KOSDAQ150)")
    logger.debug("=" * 70)
    logger.debug("")
    logger.debug("  [전략 파라미터] — 시나리오 B 최적화 (튜닝 A~G 중 CAGR 최고·MDD 기준 충족)")
    logger.debug(f"  HIGH_52W_THRESHOLD      = {HIGH_52W_THRESHOLD}  (52주 고점의 {HIGH_52W_THRESHOLD*100:.0f}% 이상)")
    logger.debug(f"  VOLUME_SPIKE            = {VOLUME_SPIKE}   (20일 평균 거래량의 {VOLUME_SPIKE}배)")
    logger.debug(f"  SURGE_EXCLUDE           = {SURGE_EXCLUDE}  (5일 내 {SURGE_EXCLUDE*100:.0f}% 급등 제외)")
    logger.debug(f"  ADX_MIN                 = {ADX_MIN}")
    logger.debug(f"  RSI_MAX                 = {RSI_MAX}")
    logger.debug(f"  ATR_MULT                = {ATR_MULT}   (트레일링 스톱)")
    logger.debug(f"  MAX_HOLD_DAYS           = {MAX_HOLD_DAYS}  (최대 보유기간)")
    logger.debug(f"  MAX_POSITIONS           = {MAX_POSITIONS}  (최대 동시 보유)")
    logger.debug(f"  FIRST_BREAKOUT_ONLY     = {FIRST_BREAKOUT_ONLY}  (최근 {FIRST_BREAKOUT_LOOKBACK}일 내 첫 돌파만)")
    logger.debug("")

    # ── [1] 데이터 로드 ──────────────────────────────────────
    logger.debug("[1] 데이터 로드 (캐시 또는 yfinance 다운로드)...")
    all_data_raw, spy_df, etf_raw, universe_map = load_full_universe(START)
    logger.debug(f"  → 종목 {len(all_data_raw)}개 로드 완료")
    ...
    for i, (t, df) in enumerate(all_data_raw.items()):
        if i % 100 == 0:
            logger.debug(f"  진행: {i}/{len(all_data_raw)}")
        all_data[t] = add_indicators(df)
    logger.debug(f"  완료: {len(all_data)}종목 지표 계산 완료")

    # ── [3] 백테스트 실행 ─────────────────────────────────────
    logger.debug(f"\n[3] 백테스트 실행 중... (기간: {START} ~ {END})")
    nav_list, trade_log, dates = run_backtest(all_data)
    logger.debug(f"  완료: {len(trade_log)}건 거래 (매수 {sum(1 for t in trade_log if t['action']=='BUY')}건 / 매도 {sum(1 for t in trade_log if t['action']=='SELL')}건)")
