...
if __name__ == "__main__":
    ...
    logger.debug("=" * 70)
    logger.debug("  52주 신고가 돌파 전략 — 파라미터 튜닝 (시나리오 A~G)")
    ...
    logger.debug(f"  → {len(all_data_raw)}개 종목 로드 완료")
    ...
    logger.debug(f"  완료: {len(all_data)}종목")
    ...
    for cfg in SCENARIOS:
        ...
        results.append(row)
        logger.debug(f"  완료: CAGR {met['CAGR']:+.1%} | MDD {met['MDD']:.1%} | 샤프 {met['샤프']:.2f} | 거래 {trade_count}건")
