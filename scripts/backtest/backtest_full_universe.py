...
def get_shares_outstanding(tickers, batch_size=50):
    ...
    for i in range(0, total, batch_size):
        ...
        print(f"\r    발행주식수: {i}/{total}", end="", flush=True)
        for t in batch:
            try:
                ...
                pass
    print(f"\r    발행주식수: {total}/{total} → {len(shares)}개 확보")
    return shares
...
def download_prices(tickers, start, end, label=""):
    ...
    for i in range(0, total, batch_size):
        batch = tickers[i:i+batch_size]
        print(f"\r    {label} 다운로드: {i}/{total}", end="", flush=True)
        try:
    ...
    print(f"\r    {label} 다운로드: {total}/{total} → {len(all_data)}개 확보")
    return all_data
...
def save_universe_cache(all_data, shares, spy_close, etf_data, us_sectors):
    ...
    print(f"  캐시 저장: {DATA_DIR} ({len(all_data)}종목)")
...
def run_backtest(all_data_ind, etf_data, spy_close, shares, sectors,
    ...
    for i, rd in enumerate(rebal_dates):
        if (i+1) % 100 == 0 or i == total-1:
            print(f"\r    {strategy_name}: {i+1}/{total} ({(i+1)/total:.0%})", end="", flush=True)
